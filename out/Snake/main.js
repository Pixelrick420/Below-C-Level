...
exports.activateSnake = activateSnake;
...
function setupAutoSnake(context) {
    const config = vscode.workspace.getConfiguration('belowCLevel');
    const autoSnake = config.get('autoSnake', false);
    if (gameTimer) {
...
function startRandomTimer(context) {
    ...
    const config = vscode.workspace.getConfiguration('belowCLevel');
    const spawnChance = config.get('snakeSpawnChance', 0.1);
